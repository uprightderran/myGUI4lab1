...
public class MyFrame extends JFrame {
	...
	private void openOptionsWindow(String path) {
		...
		String[] functions = { "Show directed graph", "Search bridge word", "Generate new text",
				"Calculate the shortest route from word1 to word2", "Calculate all the shortest route from word",
				"Random walk" };
	...
	private void performFunction(int index, String path) {
		try {
			...
			case 0: // Show directed graph
				...
				Interface.showTextGraph(graph, path.replace("GraphData.txt", ""), path.replace("GraphData.txt", ""),
						false);
